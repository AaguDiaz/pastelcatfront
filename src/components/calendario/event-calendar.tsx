...
export function EventCalendar({
  ...
  const handleEventSave = (event: CalendarEvent) => {
    if (event.id) {
      ...
      toast(`Evento "${event.title}" actualizado`, {
        description: format(new Date(event.start), "d 'de' MMM, yyyy", { locale: es }),
      ...
      toast(`Evento "${event.title}" creado`, {
        description: format(new Date(event.start), "d 'de' MMM, yyyy", { locale: es }),
        position: "bottom-left",
      })
    }
  ...
  const handleEventDelete = (eventId: string) => {
    ...
    if (deletedEvent) {
      toast(`Evento "${deletedEvent.title}" eliminado`, {
        description: format(new Date(deletedEvent.start), "d 'de' MMM, yyyy", { locale: es }),
        position: "bottom-left",
      })
    }
  }
  ...
  const handleEventUpdate = (updatedEvent: CalendarEvent) => {
    ...
    toast(`Evento "${updatedEvent.title}" movido`, {
      description: format(new Date(updatedEvent.start), "d 'de' MMM, yyyy", { locale: es }),
      position: "bottom-left",
    })
  }
      ...
      return (
        <>
          ...
          <span className="max-md:hidden">
            {format(currentDate, "EEE d 'de' MMMM, yyyy", { locale: es })}
          </span>
        </>
      )
    } else if (view === "agenda") {
  ...
  return (
    <div
      className="bg-pastel-cream flex flex-col rounded-lg border has-data-[slot=month-view]:flex-1 shadow-2xl"
      style={
        {
